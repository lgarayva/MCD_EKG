...
Las enfermedades cardiovasculares son la principal causa de muerte a nivel mundial, incluso en países con ingresos altos, donde solo el cáncer las supera. Un diagnóstico temprano y preciso mediante ECG ayuda a prevenir complicaciones graves como el infarto agudo de miocardio.

El electrocardiograma (ECG) es un estudio no invasivo que registra la actividad eléctrica del corazón. Es fundamental para detectar alteraciones en el ritmo cardiacto, conducción eléctrica y morfología cardíaca.

Interpretar un ECG requiere experiencia clínica y puede variar entre especialistas. Por eso, se han buscado modelos automatizados de interpretación usando machine learning. El dataset PTB-XL, que tiene más de 21,000 registros validados por cardiólogos, permite explorar estas aplicaciones.

Este proyecto busca desarrollar un modelo de clasificación multiclase de ECG para diferenciar distintas patologías cardíacas, usando ingeniería de variables, análisis de series temporales y algoritmos de machine learning, y evaluar su desempeño y utilidad clínica.

# 2. Datos

### 2.1 Descricpión del dataset

Los datos fueron obtenidos de PTB-XL, el cual es el mayor conjunto de datos clínicos de ECG disponible públicamente hasta la fecha. Contiene registros de 12 derivaciones de pacientes con distintas patologías cardiacas. Estos datos fueron desarrollados con el objetivo de ser utilizados en métodos de aprendizaje de máquina con el fin de generar un sistema de decisiones automatizado para la interpretación de ECG. Este conjunto de datos fue desarrollado con contrarrestar dos grandes obstáculos que se tenían con datos de ECG.

Antes de la creación del PTB-XL existían dos grandes limitaciones en los datos para la investigación de ECG. En primer lugar no se tenía un conjunto de datos para entrenamiento y validación de dominio público que se puediera usar para entrenar y validar los modelos. En segundo luhar, existía una falta de procedimientos definidos para evaluar los algoritmos.

Antes de la creación de PTB-XL, había dos grandes limitaciones en los datos para investigación de ECG. En Primer lugar, no existía un conjunto de datos público con la cantidad y calidad necesarias para entrenar y validar modelos de aprendizaje de máquina. En segundo lugar, no había procedimientos estandarizados para evaluar los algoritmos, lo que dificultaba comparar su desempeño de manera consistente.

### 2.2 características de los datos

El conjunto de datos tiene un volumen de 21,837 registros de 12 señales, cada una de 10 segundos, provemientes de 18,885 pacientes. El conjunto está balanceado respecto al género (52% hombres y 48% mujeres) y presenta un aplio rango de edades, desde 0 hasta 95 años (mediana de 62 y un rango intercuartílico de 22). Los electrocardiogramas fueron validados por hasta 2 cardiólogos, los registros incluyen información sobre ritmo, forma y diagnóstico del ECG. Los diagnósticos se clasificaron en formato de múltiples etiquetas, organizados en 5 súperclases y 24 subclases.

### 2.3 Adquisición y procesamiento de datos

Para los datos se realizó un proceso de adquisición de datos y procesamiento de datos. Los datos de PTB-XL fueron registrados mediante dispositivos Schiller AG de octubre de 1989 a junio de 1966. La información fue registrada en la base de datos por una enfermera. Cada registro fue interpretado en un 67.13% de manera manual por un cardiólogo, 31.2% de manera automática por un dispositivo de ECG con validaciones posteriores por un cardiólogo y un 1.67% sin reporte inicial. Finalmente, todos los reportes volvieron a ser anotados de manera manual por un experto basado principalmente en características cuantitativas de las señales.

Las señales se recortaron en segmentos de 10 segundos y guardado en un formato comprimido de 400 Hz. Para todas las señales, se usó el estándar de las 12 caras (I, II, III, aVL, aVR, aVF, V1, V2, V3, V4, V5 y V6) con referencia al brazo derecho. Las señales fueron convertidas del formato original a un formato binario con 16bits de precisión a una resolución de 1 $\mu$ V /LSB. Las señales pasaron por un proceso en el que se eliminaron picos de encendido y apagado en los dispositivos, estos picos se encontraban al inicio y final de los registros, además, las señales fueron re muestreadas a una señal de 500 Hz, también se generó una versión de 100 Hz.
...
* Análisis de correlación cruzada de las series originales.
* Análisis de correlación cruzada de las series originales con intervalos de desviación estándar.
* Análisis de correlación cruzada promedio de las series originales con intervalos de desviación estándar.
* Análisis de cross-correlation del promedio de series con suavizamiento (window 50) e intervalos de desviación estándar.
* Análisis de cross-correlation window particular.
* Análisis de cross-correlation con ventana (window) promedio
...
De este análisis vemos que en promedio la correlación cruzada tiene un máximo en el retraso 0, sin embargo, en los demás retrasos no parece existir una correlación cruzada significativa.
...
En relación con el análisis interior, vemos que en promedio la correlación cruzada tiene un máximo en el retraso 0, sin embargo, en los demás retrasos no parece existir una correlación cruzada significativa.

### Análisis de cross-correlation del promedio de series con suavizamiento (window 50) e intervalos de desviación estándar.

Otro análisis realizado fue el análisis de correlación cruzada del promedio de la serie suavizada con una ventana arbitraria de 50 periodos.
...
De las gráficas podemos notar que no existe una notable diferencia entre las clases, además que no se encuentra una diferencia notable con respecto a la correlación cruzada de la serie original.


### Análisis de cross-correlation window particular.

Otro análisis realizado fue el análisis de correlación cruzada utilizando como suavizamiento ventanas con periodos obtenidos en la descomposición de la serie. Este suavizamiento se hizo con los valores particulares de cada señal.
...
Podemos observar que utilizar una ventana de suavizamiento particular tenemos una gráfica de correlación cruzada más suave, y podemos notar los picos en otros puntos además del retraso cero.

Sin embargo, tampoco existe una relación clara para separar las clases de las señales.
...
### Promedio de series con intervalos de desviación estándar.

Se realizó un análisis del promedio de las series con intervalos de desviación estándar, sin embargo, visualmente no se encontraron relaciones directas para separar las clases.

![SIGNAL II MEAN](/Users/leongaray/Desktop/MCD_EKG/img/ccf/signal_II_mean.png)

Como se observa en las gráficas, las señales presentan bastante variabilidad y no se logra apreciar patrones en las clases.
...
Se realizó un análisis del promedio de las series suavizadas con una ventana de 50 periodos con intervalos de desviación estándar, sin embargo, visualmente no se encontraron relaciones directas para separar las clases.
...
Se realizó otro análisis, el promedio de las series suavizadas con una ventana particular para cada señal y clase con intervalos de desviación estándar, sin embargo, visualmente no se encontraron relaciones directas para separar las clases.
...
El último análisis realizado fue el promedio de las series suavizadas con una ventana promedio para cada señal con intervalos de desviación estándar, sin embargo, visualmente no se encontraron relaciones directas para separar las clases.
...
En este apartado se trabajó, con base en los resultados del análisis exploratorio de datos, la creación de variables para el modelo. Se trabajaron tres bloques de variables y una reducción de dimensionalidad:
...
## Variables de autocorrelaciones
...
## Variables estadísticas de las series
...
## Variables de correlación cruzada 
...
Buscando soluciones a tener una cantidad limitada de registros se optó por dividir el datasets en chunks, es decir, cada serie se dividía en pedazos para poder tener una mayor cantidad de regitros. Para esta parte se tomaron dos particiones. La primera partición se tomó el número de retrazos obtenidos en la función de autocorrelación, cinco retrazos, y la segunda partición fue una partición arbitraria de 100 regitros, es decir, tomando cada registro como un segundo de la serie.
...
|              |   precision |   recall |   f1-score |   support |
|:-------------|------------:|---------:|-----------:|----------:|
| 0            |    0.37     |     0.37 |   0.37     |    300    |
| 1            |    0.722892 |     0.6  |   0.655738 |    300    |
| 2            |    0.375    |     0.47 |   0.41716  |    300    |
| 3            |    0.48     |     0.44 |   0.45913  |    300    |
| accuracy     |    0.47     |     0.47 |   0.47     |      0.47 |
| macro avg    |    0.486973 |     0.47 |   0.475507 |   1200    |
| weighted avg |    0.486973 |     0.47 |   0.475507 |   1200    |

| metric             |    value |
|:-------------------|---------:|
| accuracy           | 0.47     |
| precision_weighted | 0.486973 |
| recall_weighted    | 0.47     |
| f1_weighted        | 0.475507 |
| roc_auc_ovr        | 0.72971  |
| log_loss           | 1.27737  |
| gini_normalized    | 0.45942  |
| ks_test_clase_0    | 0.22     |
| ks_test_clase_1    | 0.565556 |
| ks_test_clase_2    | 0.286667 |
| ks_test_clase_3    | 0.366667 |
...
|:-------------------|---------:|
| accuracy           | 0.508333 |
| precision_weighted | 0.538344 |
| recall_weighted    | 0.508333 |
| f1_weighted        | 0.516082 |
